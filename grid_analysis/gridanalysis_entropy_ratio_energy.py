...
def worker_fn(dirname, filepath):
    ...
    for ratio in ratios:
        ...
        energy.append(5/2*sum(low_entropy['cell_mass']*low_entropy['temperature']*
                      (1.0/low_entropy['temperature_ratio']-1))/0.61/mH*kB)

    return (int(ds.basename[-4:]), ds.current_time.in_units('Myr')) + tuple(energy)
...
if results:
    ...
    header = 'filenumber, t(Myr), ' + 'energy(erg)'*len(ratios)
    for dirname in collected.keys():
        ...
        if dirname.strip('/').split('/')[-1] == 'data':
            ...
            savedir = dirname
        np.savetxt(dirname+'/gridanalysis_entropy_ratio_energy.txt', np.asarray(collected[dirname]), fmt=fmt, header=header)
